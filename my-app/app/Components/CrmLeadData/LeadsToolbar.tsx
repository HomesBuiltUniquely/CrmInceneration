function Pill({
  ...
  return (
    ...
    >
      <span>{label}</span>
      {value !== undefined ? (
        <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${active ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500"}`}>
          {value}
        </span>
      ) : null}
    </button>
...
function DensityToggle() {
  return (
    <div className="flex items-center rounded-xl bg-white p-1 ring-1 ring-slate-200">
      <button className="rounded-lg bg-slate-100 px-3 py-1 text-[11px] font-semibold text-slate-700">Comfortable</button>
      <button className="rounded-lg px-3 py-1 text-[11px] font-semibold text-slate-500 hover:bg-slate-50">Compact</button>
    </div>
  );
}

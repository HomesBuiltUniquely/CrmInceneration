...
const STATUS_OPTIONS = ["Active", "Callback", "Qualified", "On Hold"];
  ...
  useEffect(() => {
    ...
    async function loadSubStatuses() {
      ...
      try {
        ...
        const data: { subStatuses?: string[] } = await response.json();
        const options = Array.isArray(data.subStatuses) ? data.subStatuses : [];

        if (!cancelled) {
          const nextOptions = options.length > 0 ? options : [lead.status];
          setFeedbackOptions(nextOptions);
          if (!nextOptions.includes(feedback)) {
            setFeedback(nextOptions[0]);
          }
        }
      } catch (error) {
        if (!cancelled) {
            ...
            <div>
              ...
              <div className="relative">
                <Select
                  ...
                  className=" h-[42px] rounded-[12px] bg-[#f4f1e8] text-[14px] pl-8"
                >
                ...
                <span className="absolute left-3 top-1/2 -translate-y-1/2 h-2.5 w-2.5 rounded-full bg-green-600" />
              </div>
            </div>

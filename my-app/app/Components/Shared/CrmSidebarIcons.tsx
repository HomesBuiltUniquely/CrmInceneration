...
const IMAGE_SIDEBAR_ICONS = new Set([
  ...
  ...Object.keys(MODULE_TILE_ICON_SRC),
]);
...
export function isModuleTileIcon(name: string): boolean {
  return name in MODULE_TILE_ICON_SRC;
}

export function CrmSidebarIcon({ name, className }: CrmSidebarIconProps) {
  const moduleSrc = MODULE_TILE_ICON_SRC[name];
  if (moduleSrc) {
    return (
      <img
        src={moduleSrc}

...
/**
 * Proxies Hub Closed new vs old breakdown.
 * GET /v1/crm/insights/closed-new-old
 */
export async function GET(req: NextRequest) {
  try {
    const qs = req.nextUrl.searchParams.toString();
    const candidates = [
      `${BASE_URL}/v1/crm/insights/closed-new-old${qs ? `?${qs}` : ""}`,
      `${BASE_URL}/api/crm/insights/closed-new-old${qs ? `?${qs}` : ""}`,
    ];
    ...
    return proxyJsonError(
      ...
      "Unable to load closed new/old breakdown.",
    );
  } catch (error) {
    return NextResponse.json(
      {
        success: false,
        error: "Unable to load closed new/old breakdown.",

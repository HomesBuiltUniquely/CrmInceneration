...
/**
 * Proxies Hub CRM Insights sales performance cards (4 KPI tiles).
 * GET /v1/crm/insights/performance-cards
 */
export async function GET(req: NextRequest) {
  try {
    const qs = req.nextUrl.searchParams.toString();
    const candidates = [
      `${BASE_URL}/v1/crm/insights/performance-cards${qs ? `?${qs}` : ""}`,
      `${BASE_URL}/api/crm/insights/performance-cards${qs ? `?${qs}` : ""}`,
    ];
    ...
    return proxyJsonError(
      ...
      "Unable to load CRM insights performance cards.",
    );
  } catch (error) {
    return NextResponse.json(
      {
        success: false,
        error: "Unable to load CRM insights performance cards.",

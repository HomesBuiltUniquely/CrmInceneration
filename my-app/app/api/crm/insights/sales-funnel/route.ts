...
/**
 * Proxies Hub CRM Insights sales funnel (current | passages | cohort).
...
export async function GET(req: NextRequest) {
  try {
    ...
    const candidates = [
      `${BASE_URL}/v1/crm/insights/sales-funnel${qs ? `?${qs}` : ""}`,
      `${BASE_URL}/api/crm/insights/sales-funnel${qs ? `?${qs}` : ""}`,
    ];
    ...
    for (const url of candidates) {
      ...
      // 501 = passages history unavailable — forward body so FE can show passagesAvailable:false
    ...
    return proxyJsonError(
      ...
      "Unable to load CRM insights sales funnel.",
    );
  } catch (error) {
    return NextResponse.json(
      {
        success: false,
        error: "Unable to load CRM insights sales funnel.",
